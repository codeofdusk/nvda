...
from ctypes import ArgumentError, CDLL, c_int, wintypes  # noqa: I001
import threading
...
def _initializeExternal() -> CDLL | None:
	...
	try:
		...
		log.exception("Could not load or start the winEventLimiter dll")
		return None
	if res != 0:
...
def _registerInternalHooks():
	...
	for eventType, eventName in winEventIDsToNVDAEventNames.items():
		hookID = winUser.setWinEventHook(eventType, eventType, 0, cWinEventCallback, 0, 0, 0)
		if hookID:
			winEventHookIDs.append(hookID)
		else:
			log.error(
				f"initialize: could not register callback for event {eventType} ({eventName})",
			)
